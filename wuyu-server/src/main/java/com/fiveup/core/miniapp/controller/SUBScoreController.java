...
import com.fiveup.core.miniapp.common.SUBStandard;
 ...
 * @author 史林
 * @date 2022/10/10
 * @function 仰卧起坐测量值接收，按照年级、性别的不同标准，得出分数与等级
 */
@RestController
...
public class SUBScoreController {
    ...
    @GetMapping("/getSUBScoreList")
    public CommonResponse<List<StuScore>> getSUBScoreList(@RequestParam String grade,
...
//        }
        stuScoreList = subScoreService.getSUBScoreList();
        return CommonResponse.ok(stuScoreList);
    }

    //肺活量
    @PostMapping(value="/addSUBScore")
    public CommonResponse addStudentSportScore(@RequestBody ScoreSport<Integer> scoreSportFront) {
      ...
      System.out.println(scoreSport);
        ...
        boolean flag;
        if (subScoreService.isRecordExist(scoreSport.getStudentNum())) {
            flag = subScoreService.updateSportScore(scoreSport);
            return  flag ? CommonResponse.ok():CommonResponse.fail(10001,"更新数据失败");
        }
        else{
            flag = subScoreService.insertSportScore(scoreSport);
    ...
    public ScoreSport setScore(ScoreSport<Integer> scoreSport) {
        ...
        // 根据年级获得对应的得分标准
        switch (grade) {
            case "3": standard = SUBStandard.getThree(); break;
            case "4": standard = SUBStandard.getFour(); break;
            case "5": standard = SUBStandard.getFive(); break;
            case "6": standard = SUBStandard.getSix(); break;
            default: return null;
        }
