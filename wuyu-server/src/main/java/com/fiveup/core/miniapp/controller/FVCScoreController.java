...
import com.fiveup.core.miniapp.common.FVCStandard;
 ...
 * @author 史林
 * @date 2022/10/10
 * @function 肺活量指标测量值接收，按照年级、性别的不同标准，得出分数与等级
 */
@RestController
@RequestMapping("/minapp")
public class FVCScoreController {

    @Autowired
    private FVCScoreService fvcScoreService;

    @GetMapping("/getFVCScoreList")
    public CommonResponse<List<StuScore>> getStudentScoreList(@RequestParam String grade,
...
//        }
        stuScoreList = fvcScoreService.getFVCScoreList();
        return CommonResponse.ok(stuScoreList);
    }

    //肺活量
    @PostMapping(value="/addFVCScore")
...
//      System.out.println(scoreSport);
        ...
        boolean flag;
        if (fvcScoreService.isRecordExist(scoreSport.getStudentNum())) {
            flag = fvcScoreService.updateSportScore(scoreSport);
            return  flag ? CommonResponse.ok():CommonResponse.fail(10001,"更新数据失败");
        }
        else{
            flag = fvcScoreService.insertSportScore(scoreSport);
    ...
    public ScoreSport setScore(ScoreSport<Integer> scoreSport) {
        ...
        // 根据年级获得对应的得分标准
        switch (grade) {
            case "1": standard = FVCStandard.getOne(); break;
            case "2": standard = FVCStandard.getTwo(); break;
            case "3": standard = FVCStandard.getThree(); break;
            case "4": standard = FVCStandard.getFour(); break;
            case "5": standard = FVCStandard.getFive(); break;
            case "6": standard = FVCStandard.getSix(); break;
            default: return null;
        }
