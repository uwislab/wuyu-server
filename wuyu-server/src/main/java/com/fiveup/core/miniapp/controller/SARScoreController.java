...
import com.fiveup.core.miniapp.common.SARStandard;
 ...
 * @author 史林
 * @date 2022/10/10
 * @function 坐位体前屈测量值接收，按照年级、性别的不同标准，得出分数与等级
 */
@RestController
@RequestMapping("/minapp")
public class SARScoreController {

    @Autowired
    private SARScoreService sarScoreService;

    @GetMapping("/getSARScoreList")
    public CommonResponse<List<StuScore>> getSARScoreList(@RequestParam String grade,
...
//        }
        stuScoreList = sarScoreService.getSARScoreList();
        return CommonResponse.ok(stuScoreList);
    }

    //肺活量
    @PostMapping(value="/addSARScore")
...
//      System.out.println(scoreSport);
        ...
        boolean flag;
        if (sarScoreService.isRecordExist(scoreSport.getStudentNum())) {
            flag = sarScoreService.updateSportScore(scoreSport);
            return  flag ? CommonResponse.ok():CommonResponse.fail(10001,"更新数据失败");
        }
        else{
            flag = sarScoreService.insertSportScore(scoreSport);
    ...
    public ScoreSport setScore(ScoreSport<Double> scoreSport) {
        ...
        // 根据年级获得对应的得分标准
        switch (grade) {
            case "1": standard = SARStandard.getOne(); break;
            case "2": standard = SARStandard.getTwo(); break;
            case "3": standard = SARStandard.getThree(); break;
            case "4": standard = SARStandard.getFour(); break;
            case "5": standard = SARStandard.getFive(); break;
            case "6": standard = SARStandard.getSix(); break;
            default: return null;
        }
